...
def load_audio(audio_path: str, sampling_rate: int):
    """
    Load audio without relying on ``torchaudio.load``.

    TorchAudio 2.9+ delegates all decoding to TorchCodec, making a simple WAV
    reference fail when TorchCodec is not installed. Voice Clone already ships
    libsndfile through ``soundfile``, so use that stable decoder first and keep
    pydub/FFmpeg only as a fallback for container formats libsndfile cannot read.
    ...
    try:
        ...
        # soundfile returns (time, channels); OmniVoice expects (channels, time).
        waveform = torch.from_numpy(np.ascontiguousarray(audio_data.T))
    except (RuntimeError, OSError, ValueError) as soundfile_error:
        # Fallback via the bundled imageio-ffmpeg executable for formats such
        # as M4A/WebM that libsndfile may not support. Decode straight to mono
        # float32 PCM so this path does not require pydub's separate ffprobe
        # executable.
        try:
            ...
            audio_data = np.frombuffer(decoded.stdout, dtype="<f4").copy()
            if audio_data.size == 0:
                raise RuntimeError("FFmpeg không trả về dữ liệu audio.")
            waveform = torch.from_numpy(audio_data).unsqueeze(0)
            prompt_sampling_rate = sampling_rate
        except Exception as ffmpeg_error:
            ffmpeg_detail = getattr(ffmpeg_error, "stderr", b"")
            if isinstance(ffmpeg_detail, bytes):
                ffmpeg_detail = ffmpeg_detail.decode("utf-8", errors="replace").strip()
            raise RuntimeError(
                "Không thể đọc audio tham chiếu. Hãy dùng WAV hoặc FLAC hợp lệ; "
                f"chi tiết: {soundfile_error}; FFmpeg: {ffmpeg_detail or ffmpeg_error}"
            ) from ffmpeg_error
